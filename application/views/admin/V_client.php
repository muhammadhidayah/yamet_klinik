			...
			<div class="col-md-9 menutengah">
					...
					<div class="table-responsive">
						<table class="table">
							...
							<th colspan="2"><a href="<?php echo site_url('admin/client/tambah_client')?>"><button type="button" class="btn btn-success">++ Add Client</button></a></th>
							</tr>
							...
							<?php $no=0; foreach ($query as $list) { $no++; ?>
								
						
							<tr>
								<td><?php echo $no ?></td>
								<td><?php echo $list->nama_client ?></td>
								<td><?php echo $list->diagnosis_client ?></td>
								<td><?php echo $list->nama_ayahclient ?></td>
								<td><?php echo $list->alamat_client ?></td>
								<td><a href="<?php echo site_url('admin/client/resetpassword/'.$list->id_client) ?>" title="Reset Password User" class="btn btn-warning btn-sm"> Reset</a></td>
								<td><a href="<?php echo site_url('admin/client/editclient/'.$list->id_client) ?>" class="btn btn-success btn-sm"> Edit</a></td>
								<td>
									<?php if($list->status_client == "aktif") { ?>
										<a href="<?php echo site_url('admin/client/updateStatus/'.$list->id_client) ?>" title="Reset Password User" class="btn btn-danger btn-sm"> Non-Aktifkan User</a>
									<?php } else { ?>
										<a href="<?php echo site_url('admin/client/updateStatus/'.$list->id_client) ?>" title="Reset Password User" class="btn btn-success btn-sm"> Aktifkan User</a>
									<?php } ?>
								</td>
							</tr>
								<?php } ?>
